...
- **模拟器 Emulators** — 模拟某个真实主机/硬件的软件。按厂商归类，再按机型放入二级分类（如任天堂 → SNES / N64 / Switch 等；索尼 → PS1/PS2/PSP 等）。跨多平台的放「多系统 / 通用」。街机放「街机 Arcade」，老式 PC/家用计算机放「复古计算机」。
- **FPGA 硬件模拟 FPGA Emulation & Cores** — 用 FPGA 在硬件层面重建主机/街机的开源平台与核心（MiSTer、MiST、JT cores、Analogue Pocket openFPGA 核心等）。与软件「模拟器」区分：软件模拟归模拟器，硬件电路重建归此类。
...
- **开源复古硬件 Retro Hardware & DIY** — 面向复古主机/街机的开源硬件与固件（卡带备份、音源、扫描转换、手柄适配；sanni/cartreader、mt32-pi、OSSC、BlueRetro 等）。注意：FPGA 平台/核心归「FPGA 硬件模拟」，不放这里。
